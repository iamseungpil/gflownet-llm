...
class ARCLLMExperiment:
    ...
    def create_baseline_prompt(self, task: Dict, num_examples: Optional[int] = None) -> str:
...
"""
    ...
    def create_augmented_prompt(self, task: Dict, augmented_examples: List[Dict], 
                               augmentation_type: str, num_augmented: int) -> str:
...
"""
    ...
    def evaluate_with_candidates(self, task: Dict, prompt: str, 
                                num_candidates: int = 5) -> Tuple[float, List[bool]]:
        ...
        for _ in range(num_candidates):
            ...
            predicted_grid = self.extract_grid_from_response(response)
    ...
    def run_data_scaling_experiment(self, task_id: str = "6150a2bd", 
        ...
        for num_examples in baseline_sizes:
            ...
            prompt = self.create_baseline_prompt(task, num_examples)
            accuracy, predictions = self.evaluate_with_candidates(task, prompt, num_candidates)
        ...
        if rearc_examples:
            ...
            for num_augmented in range(0, max_augmented + 1):
                ...
                prompt = self.create_augmented_prompt(task, rearc_examples, "RE-ARC", num_augmented)
                accuracy, predictions = self.evaluate_with_candidates(task, prompt, num_candidates)
        ...
        if harc_examples:
            ...
            for num_augmented in range(0, max_augmented + 1):
                ...
                prompt = self.create_augmented_prompt(task, harc_examples, "H-ARC", num_augmented)
                accuracy, predictions = self.evaluate_with_candidates(task, prompt, num_candidates)
...
def main():
    ...
    #     experiment = ARCLLMExperiment(use_wandb=True)
    #     results = experiment.run_data_scaling_experiment(task_id)
